...
class OrcamentoEquipAdapter(
    private val onEditar: (OrcamentoEquipLinha) -> Unit,
    private val onRemover: (OrcamentoEquipLinha) -> Unit
) : RecyclerView.Adapter<OrcamentoEquipAdapter.LinhaViewHolder>() {

    private var itens: List<OrcamentoEquipLinha> = emptyList()
    private val currency = NumberFormat.getCurrencyInstance(Locale("pt", "BR"))

    fun update(novos: List<OrcamentoEquipLinha>) {
        itens = novos
    ...
    inner class LinhaViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(linha: OrcamentoEquipLinha) {
            tvNome.text = linha.nome
            tvDetalhe.text =
                "${linha.periodo.label} • ${linha.quantidade} x ${currency.format(linha.valorUnitario)}"
