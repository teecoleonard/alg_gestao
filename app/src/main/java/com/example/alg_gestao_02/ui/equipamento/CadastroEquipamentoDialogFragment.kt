...
class CadastroEquipamentoDialogFragment : DialogFragment() {
    ...
    private lateinit var tilFotoUrl: TextInputLayout
    ...
    private fun initViews(view: View) {
        ...
        tilFotoUrl = view.findViewById(R.id.tilFotoUrl)
    ...
    private fun parseCurrencyValue(text: String): Double {
        return try {
            ...
            LogUtils.error("CadastroEquipamentoDialog", "Erro ao converter valor monetário", e)
            0.0
        }
    }
    ...
    private fun preencherCampos(equipamento: Equipamento) {
        ...
        etFotoUrl.setText(equipamento.fotoUrl ?: "")
    ...
    private fun validarCampos(): Boolean {
        ...
        val fotoUrl = etFotoUrl.text?.toString()?.trim().orEmpty()
        if (fotoUrl.isNotBlank() && !Patterns.WEB_URL.matcher(fotoUrl).matches()) {
            tilFotoUrl.error = "Informe uma URL válida (http/https)"
            isValid = false
        } else {
            tilFotoUrl.error = null
        }
    ...
    private fun salvarEquipamento() {
        try {
            ...
            val fotoUrl = etFotoUrl.text?.toString()?.trim()?.takeIf { it.isNotBlank() }
            ...
            LogUtils.error("CadastroEquipamentoDialog", "Erro ao salvar equipamento", e)
